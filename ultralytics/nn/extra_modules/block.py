...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'LAWDS', 'CPMAonv', 'EMSConvP', 'C3_EMSC', 'C3_EMSCP', 'C2f_CPMA', 'C2f_EMSCP', 'RCSOSA', 'C3_KW', 'C2f_KW',
           ...
           'C3_VSS', 'C2f_VSS', 'C3_LVMB', 'C2f_LVMB', 'RepNCSPELAN4', 'DBBNCSPELAN4', 'OREPANCSPELAN4', 'DRBNCSPELAN4', 'LDSS', 'V7DownSampling', 'CBLinear', 'CBFuse', 'Silence',
           ...
           'C2f_GroupMambaBlock', 'C2f_MambaVision'
           ]
...
class CPMAonv(nn.Module):
    # Efficient Multi-Scale Conv
    def __init__(self, channel=256, kernels=[5, 7]):
        super().__init__()
...
class Bottleneck_CPMA(Bottleneck):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = CPMAConv(c2)

class C3_EMSC(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_CPMA(c_, c_, shortcut, g, k=((1, 1), (3, 3)), e=1.0) for _ in range(n)))

class C2f_CPMA(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_CPMA(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
...
class FocusFeature(nn.Module):
    def __init__(self, inc, kernel_sizes=(3, 5, 7, 9), e=0.5) -> None:
        super().__init__()
        hidc = int(inc[1] * e)
